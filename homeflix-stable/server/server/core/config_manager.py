...
class Settings(BaseModel):
    ...
    @field_validator("video_directories")
    @classmethod
    def validate_video_dirs(cls, dirs):
        """Valide les répertoires vidéo sans les filtrer automatiquement."""
        # Ne plus filtrer automatiquement - permet la détection automatique
        return [str(Path(d).absolute()) for d in dirs]
...
def load_settings() -> Settings:
    ...
    try:
        ...
        logger.error(f"[ERREUR] Impossible de charger settings.yaml : {e}")
        return Settings()
...
def save_settings(settings: Settings) -> bool:
    ...
    try:
        ...
        logger.error(f"[ERREUR] Impossible de sauvegarder settings.yaml : {e}")
        return False
