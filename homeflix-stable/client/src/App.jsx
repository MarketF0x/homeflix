...
// Désactivation du lazy loading pour VideoDetail (régression de chargement dynamique)
...
export default function App() {
  ...
  // Détecter l'environnement Electron pour désactiver WebGL si instable
  const isElectron = typeof window !== 'undefined' && !!window.homeflix;
  const enableWebGL = useMemo(() => {
    try {
      const flag = localStorage.getItem('enableWebGL');
      return flag === '1';
    } catch { return false; }
  }, []);
  ...
  const handleCloseVideo = useCallback(() => {
    ...
    // Ne PAS recharger la page principale ni le carrousel à la fermeture
    // La page se met à jour uniquement quand l'utilisateur déclenche un scan/refresh manuel
  ...
  const handleSelectVideo = useCallback((video) => {
    // Version simplifiée fiable
    scrollPositionRef.current = window.scrollY || window.pageYOffset;
    try { window.history.pushState({ video: video?.id }, "", ""); } catch {}
  ...
  return (
    <div className="app">
      {/* Désactiver le fond WebGL par défaut dans Electron pour éviter des crashes GPU; activable via localStorage.enableWebGL = '1' */}
      {(!isElectron || enableWebGL) && <WebGLBackground />}
