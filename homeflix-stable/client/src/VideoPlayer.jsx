...
export default function VideoPlayer({ video, onClose, currentProfile }) {
  ...
  // Tous les autres formats (MKV, AVI, MOV, FLV, WMV, etc.) passent par le transcodage FFmpeg
  // 🎬 FORMATS SUPPORTÉS EN DIRECT : .mp4, .webm
  // 🔄 FORMATS NÉCESSITANT TRANSCODAGE : .mkv, .avi, .mov, .flv, .wmv, .mpg, .mpeg, .m4v, .3gp, .divx, .xvid
  ...
  console.log("🎬 VideoPlayer - Vidéo:", video.title || video.path);
  console.log("🎬 VideoPlayer - Mode transcodage initial:", needsTranscodeByDefault ? "OUI" : "NON");
  ...
  const preferHls = true; // Prototype: privilégier HLS pour le transcodage
  ...
  const [hlsSid, setHlsSid] = useState(null);
  ...
  // Chargement dynamique de hls.js (CDN) pour prototype
  ...
  // Démarrer/arrêter une session HLS côté serveur lorsque transcodage est actif
  useEffect(() => {
    let cancelled = false;
    const start = async () => {
      if (!useTranscode || !preferHls) {
        // Arrêt éventuel d'une session HLS existante
        if (hlsSid) {
          try { await fetch(`${baseUrl}/api/stream/hls/stop?sid=${hlsSid}`); } catch {}
          setHlsSid(null); setHlsUrl(null); setIsHlsActive(false);
      ...
      try {
        // Lancer session HLS
        const resp = await fetch(`${baseUrl}/api/stream/hls/start?path=${encodeURIComponent(video.path)}&audio_track=${selectedAudioTrack}&quality=${transcodeQuality}`);
        if (!resp.ok) {
          // Essayer de lire le corps pour un message d'erreur utile (JSON ou texte)
          let detail = '';
          try {
          ...
          console.warn('⚠️ Impossible de démarrer HLS (server):', resp.status, detail);
          throw new Error(`HLS start HTTP ${resp.status}${detail ? ' - ' + detail : ''}`);
        }
        const data = await resp.json();
        if (cancelled) return;
        const playlistUrl = `${baseUrl}${data.playlist_url}`;
        setHlsSid(data.sid);
        ...
        // hls.js pour autres navigateurs
        const HlsLib = await loadHlsJs();
        if (HlsLib && HlsLib.isSupported()) {
          if (hlsRef.current) { hlsRef.current.destroy?.(); hlsRef.current = null; }
          const hls = new HlsLib({ maxBufferLength: 30, backBufferLength: 30 });
          hlsRef.current = hls;
          ...
          hls.on(HlsLib.Events.ERROR, (evt, data) => {
            console.warn('⚠️ HLS.js error:', data?.type, data?.details, data?.fatal);
            if (data?.fatal) {
              try { hls.destroy(); } catch {}
              hlsRef.current = null;
              setIsHlsActive(false);
              // Fallback sur MP4 transcodé
              setUseTranscode(true);
            }
          });
          setIsHlsActive(true);
        } else {
          // Fallback MP4 transcodé
          setIsHlsActive(false);
        }
      } catch (e) {
        console.warn('⚠️ Impossible de démarrer HLS, fallback MP4:', e);
        setIsHlsActive(false);
      }
    };
    start();
  ...
  }, [useTranscode, preferHls, video.path, selectedAudioTrack, transcodeQuality]);

  // Stopper la session HLS à la fermeture du lecteur
  useEffect(() => {
    return () => {
      if (hlsSid) {
        fetch(`${baseUrl}/api/stream/hls/stop?sid=${hlsSid}`).catch(()=>{});
      }
  ...
  const saveProgress = useCallback(async (position) => {
    try {
      ...
      if (!videoId) {
        console.warn("⚠️ Aucun ID vidéo, progression non sauvegardée");
        return;
      }
      
      console.log(`💾 Sauvegarde progression: vidéo ${videoId}, position ${position}s, profil ${profileId}`);
  ...
  useEffect(() => {
    const fetchSavedPosition = async () => {
      ...
      if (!profileId || !videoId) {
        console.log('⚠️ Pas de profil ou vidéo ID, pas de chargement de progression');
        return;
      }
      
      try {
        const response = await fetch(`${API}/progress/${videoId}?profile_id=${profileId}`);
        if (response.ok) {
          const data = await response.json();
          if (data.position && data.position > 0) {
            setSavedPosition(data.position);
            setCurrentTime(data.position);
            console.log(`✅ Position chargée: ${data.position}s`);
          }
        }
      } catch (error) {
  ...
  useLayoutEffect(() => {
    ...
    if (!videoElement) {
      console.warn('⚠️ videoRef.current est null dans useLayoutEffect !');
      return;
    }

    console.log('🔄 Chargement initial de la vidéo - Mode:', useTranscode ? 'TRANSCODAGE' : 'DIRECT');
    console.log('   Audio track:', selectedAudioTrack);
    console.log('   Subtitle track:', selectedSubtitleTrack);
    console.log('   Video URL:', videoUrl);
    
    // ✅ CHARGER LA VIDÉO DÈS LE MONTAGE
    videoElement.load();
    setIsLoading(true);
    setLoadError(null);
    ...
    const handleLoadedMetadata = () => {
      ...
      // ================================================================================
      // 🎯 DÉTECTION INTELLIGENTE : Codec vidéo non supporté
      // ================================================================================
      // Si width/height = 0, le codec vidéo n'est pas supporté nativement par le navigateur
      if (videoElement.videoWidth === 0 || videoElement.videoHeight === 0) {
        console.error('❌ CODEC VIDÉO NON SUPPORTÉ : Dimensions = 0x0');
        console.log(`   Fichier: ${video.path}`);
        console.log(`   Mode actuel: ${useTranscode ? 'TRANSCODAGE' : 'DIRECT'}`);
        
        // FALLBACK AUTOMATIQUE : Basculer vers transcodage si en mode direct
        if (!useTranscode && retryCountRef.current === 0) {
          console.log('🔄 FALLBACK AUTOMATIQUE : Activation du transcodage FFmpeg...');
          setLoadInfo('🔄 Format vidéo non supporté - activation du transcodage intelligent...');
          setUseTranscode(true);
          retryCountRef.current = 1; // Marquer qu'on a essayé 1 fois
          return;
        } 
        
        // Si déjà en transcodage et toujours problème = codec vraiment non supporté
        if (useTranscode) {
          setLoadError('❌ Codec vidéo incompatible même après transcodage. Veuillez vérifier FFmpeg.');
          console.error('❌ ERREUR CRITIQUE : Transcodage échoue - vérifier configuration FFmpeg');
        }
      } else {
        console.log(`✅ Dimensions vidéo valides: ${videoElement.videoWidth}x${videoElement.videoHeight}`);
        retryCountRef.current = 0; // Réinitialiser le compteur si lecture OK
      }
    };
    ...
    // ✅ GESTION AVANCÉE DES ERREURS : Diagnostic précis + Fallback intelligent
    const handleVideoError = (error) => {
      const videoError = videoElement.error;
      
      // Détails de l'erreur
      let errorMsg = '❌ Erreur de lecture vidéo';
      let errorCode = videoError?.code || 'UNKNOWN';
      
      if (videoError) {
        switch (videoError.code) {
          case 1: // MEDIA_ERR_ABORTED
            errorMsg = '❌ Chargement interrompu par l\'utilisateur';
            console.warn('Erreur: MEDIA_ERR_ABORTED');
            return; // Pas de retry si l'utilisateur a annulé
            
          case 2: // MEDIA_ERR_NETWORK
            errorMsg = '❌ Erreur réseau - Vérifiez votre connexion';
            console.error('Erreur: MEDIA_ERR_NETWORK');
            break;
            
          case 3: // MEDIA_ERR_DECODE
            errorMsg = '📹 Format vidéo non supporté même avec transcodage';
            console.error('Erreur: MEDIA_ERR_DECODE - Codec incompatible');
            console.error('   Fichier:', video.path);
            console.error('   Mode actuel:', useTranscode ? 'TRANSCODAGE' : 'DIRECT');
            
            // Si en mode direct ET pas encore essayé transcodage
            if (!useTranscode && retryCountRef.current === 0) {
              console.log('🔄 FALLBACK: Tentative avec transcodage FFmpeg...');
              setLoadInfo('🔄 Tentative de transcodage pour résoudre le problème...');
              setUseTranscode(true);
              retryCountRef.current = 1;
              return;
            }
            
            // Si déjà en transcodage = problème sérieux
            if (useTranscode) {
              errorMsg = '❌ Codec vidéo incompatible - Vérifiez le fichier avec FFprobe';
              setLoadError(errorMsg + '\n\nConseil: Testez avec ffprobe -i "' + video.path + '"');
              return;
            }
            break;
            
          case 4: // MEDIA_ERR_SRC_NOT_SUPPORTED
            errorMsg = '❌ Format source non supporté';
            console.error('Erreur: MEDIA_ERR_SRC_NOT_SUPPORTED');
            console.error('   Fichier:', video.path);
            console.error('   Extension:', videoExt);
            console.error('   URL:', videoUrl);
            console.error('   Mode actuel:', useTranscode ? 'TRANSCODAGE' : 'DIRECT');
            
            // 🔍 DIAGNOSTIC : Vérifier si l'extension nécessite un transcodage
            const needsTranscode = !['mp4', 'webm'].includes(videoExt);
            console.log('   Extension nécessite transcodage?', needsTranscode);
            console.log('   Transcodage actif?', useTranscode);
            
            // Basculer vers transcodage si pas déjà actif
            if (!useTranscode && retryCountRef.current === 0) {
              console.log('🔄 FALLBACK: Activation transcodage...');
              console.log('   → Ce format (' + videoExt.toUpperCase() + ') n\'est pas supporté nativement');
              console.log('   → FFmpeg va transcoder vers H.264 compatible navigateur');
              setLoadInfo('🔄 Format .' + videoExt + ' non supporté - activation du transcodage H.264...');
              setUseTranscode(true);
              retryCountRef.current = 1;
              return;
            }
            
            // Si déjà en transcodage ET toujours erreur = problème FFmpeg
            if (useTranscode) {
              errorMsg = '❌ Échec du transcodage FFmpeg';
              console.error('   ⚠️ Le transcodage FFmpeg a échoué ou le stream est invalide');
              console.error('   → Vérifiez les logs serveur pour plus de détails');
              setLoadError(errorMsg + '\n\n🔧 Vérifiez les logs serveur (terminal) pour voir l\'erreur FFmpeg exacte');
              return;
            }
            break;
            
          default:
            errorMsg = `❌ Erreur inconnue (code ${errorCode})`;
        }
      }
      
      console.error(errorMsg, error);
      
      // Retry exponentiel pour erreurs réseau
      if (videoError?.code === 2 && retryCountRef.current < 5) {
        ...
        setLoadError(errorMsg + ' (après 5 tentatives)');
      } else {
        setLoadError(errorMsg);
      }
    };
    ...
    if (playPromise !== undefined) {
      playPromise
        .then(() => {
          ...
          // ✅ VÉRIFICATION SUPPLÉMENTAIRE : Détecter codec incompatible après 3s
          setTimeout(() => {
            if (videoElement && (videoElement.videoWidth === 0 || videoElement.videoHeight === 0)) {
              console.error('❌ CODEC INCOMPATIBLE détecté après 3s de lecture');
              console.log(`   Fichier: ${video.title || video.path}`);
              
              // Basculer automatiquement vers transcodage si pas déjà actif
              if (!useTranscode && retryCountRef.current === 0) {
                console.log('🔄 FALLBACK RETARDÉ : Basculement vers transcodage FFmpeg...');
                setLoadInfo('🔄 Codec incompatible - basculement automatique vers transcodage...');
                setUseTranscode(true);
                retryCountRef.current = 1;
              } else if (useTranscode) {
                setLoadError('❌ Impossible de lire cette vidéo. Codec non supporté.');
              }
            } else if (videoElement) {
              console.log(`✅ Lecture stable - Résolution: ${videoElement.videoWidth}x${videoElement.videoHeight}`);
            }
          }, 3000);
        })
        .catch((error) => {
    ...
    const stalledCheckInterval = setInterval(() => {
      if (!videoElement.paused && !videoElement.ended) {
        // ✅ NE DÉTECTER LES BLOCAGES QUE SI LA VIDÉO A VRAIMENT COMMENCÉ
        // readyState >= 3 = HAVE_FUTURE_DATA (données suffisantes pour jouer)
        const canDetectStall = videoElement.currentTime > 0 && videoElement.readyState >= 3;
        
        if (canDetectStall && videoElement.currentTime === lastTime) {
          stalledCount++;
