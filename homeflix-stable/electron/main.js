...
const SERVER_PORT = 8001; // Port 8001 pour version STABLE (8000 réservé pour DEV)
const SERVER_URL = `http://127.0.0.1:${SERVER_PORT}`; // Forcer IPv4 au lieu de localhost

// OPTIMISATIONS ELECTRON
// Option de sécurité: possibilité de désactiver l'accélération matérielle si instable
...
function startServer() {
  ...
  // Utiliser la base de données partagée avec la version DEV
  // Cela permet de partager les comptes, films et progression
  const sharedDbPath = path.join(__dirname, '..', 'server', 'homeflix.db');
  ...
  console.log('   Base de donnees partagee:', sharedDbPath);
  ...
  serverProcess = spawn(pythonCmd, [serverPath], {
    ...
    env: { 
      ...
      DB_PATH: sharedDbPath  // Passer le chemin de la BDD partagée
    },
    shell: isWindows
  });
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      devTools: true, // Activer DevTools temporairement pour diagnostiquer la régression UI
    },
  ...
  const menuTemplate = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          label: 'Outils de développement (fenêtre séparée)',
          accelerator: 'F12',
          click: () => {
            try {
              ...
              if (mainWindow.webContents.isDevToolsOpened()) {
                ...
                // Ouvrir en mode détaché pour éviter les crashes GPU liés au dock
                mainWindow.webContents.openDevTools({ mode: 'detach' });
              }
            } catch (e) {
              console.warn('[DEVTOOLS] Impossible d\'ouvrir/fermer DevTools:', e.message);
            }
          }
        },
      ]
          ...
          click: () => {
            ...
            if (fs.existsSync(helpFile)) {
              const helpWin = new BrowserWindow({
                ...
                modal: !!mainWindow, // au-dessus de la fenêtre principale si dispo
                show: false, // attendre ready-to-show pour éviter un flash et garantir la visibilité
                webPreferences: { contextIsolation: true, devTools: false }
              });
              helpWin.webContents.on('did-fail-load', (_e, _code, desc) => {
                console.warn('[HELP] did-fail-load:', desc);
              });
              helpWin.once('ready-to-show', () => {
                try { helpWin.center(); } catch {}
                helpWin.show();
                try { helpWin.focus(); } catch {}
              });
  ...
  // Charger l'URL du serveur SANS cache
  mainWindow.loadURL(SERVER_URL, {
    extraHeaders: 'pragma: no-cache\n'
  }).then(() => {
    // Double reload pour vraiment vider le cache
    setTimeout(() => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        console.log('[CACHE] Premier rechargement sans cache');
        mainWindow.webContents.session.clearCache().then(() => {
          console.log('[CACHE] Cache vidé, rechargement final');
          mainWindow.webContents.reloadIgnoringCache();
        });
      }
    }, 1000);
